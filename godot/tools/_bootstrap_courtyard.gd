...
func _build_park_furniture(root: Node3D) -> void:
	...
	for bench in [
		# Plaza west edge, facing the chalk circle. Unmoved from the
		# openness pass's own position (tools/_probe_reachability.gd's
		# Bench landmark) -- what changed is that it now stands on paving
		# at the plaza's edge and is turned to face what it looks at.
		[-7.0, -9.8, 90.0],
