...
whoosh, the sand pat, the swing creak, the bench settling under a child who
sits down — and, since 2026-08-30, the background music: an eight-voice
sustained pad and a sparse music-box melody, both generated by the same
file.

`godot/tools/_probe_oneshots.gd` renders every one-shot cue to `.wav` and
reports its length, level, envelope endpoints and worst discontinuity, so
the short sounds can be listened to and inspected rather than assumed.
